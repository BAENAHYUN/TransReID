...
class QdrantStore:
    ...
    # detection 단계에서 media_type / track_id 등을 extra로 넘기는 호출부 허용.
    ...
    @classmethod
    def _get_media_type(cls, det: Any) -> str:
        """
        이미지/동영상 point 구분.

        우선순위:
          1) det.media_type
          2) det.extra["media_type"]
          3) video 정보 또는 track_id 존재 -> "video"
          4) 그 외 -> "image"

        허용값은 "image", "video" 두 개로 고정한다.
        """
        explicit = cls._normalize_id(getattr(det, "media_type", None))

        extra = getattr(det, "extra", None)
        extra_map = extra if isinstance(extra, dict) else {}
        from_extra = cls._normalize_id(extra_map.get("media_type"))

        if explicit is not None and from_extra is not None:
            if explicit.lower() != from_extra.lower():
                raise ValueError(
                    f"media_type 충돌: det.media_type={explicit!r}, "
                    f"det.extra['media_type']={from_extra!r}"
                )

        resolved = explicit if explicit is not None else from_extra

        if resolved is None:
            has_video = cls._normalize_id(extra_map.get("video")) is not None
            has_track = (
                getattr(det, "track_id", None) is not None
                or extra_map.get("track_id") is not None
            )
            resolved = "video" if (has_video or has_track) else "image"

        resolved = str(resolved).strip().lower()

        if resolved not in {"image", "video"}:
            raise ValueError(
                f"media_type은 'image' 또는 'video'여야 합니다: {resolved!r}"
            )
