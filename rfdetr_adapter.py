...
def from_rfdetr(
    ...
    for r in crop_results:
        ...
        extra["bbox_space"] = str(r.get("bbox_space", "frame"))
        ...
        explicit_media_type = r.get("media_type")
        if explicit_media_type is not None:
            media_type = str(explicit_media_type).strip().lower()
        else:
            has_video_metadata = any(
                r.get(key) is not None
                for key in ("video", "track_key", "timestamp")
            )
            has_track_metadata = (
                (track_id_key is not None and r.get(track_id_key) is not None)
                or r.get("track_id") is not None
            )
            media_type = "video" if (has_video_metadata or has_track_metadata) else "image"

        if media_type not in {"image", "video"}:
            raise ValueError(
                f"media_type은 'image' 또는 'video'여야 합니다: {media_type!r}"
            )
        ...
        if track_id_key:
            resolved_track_id = r.get(track_id_key)
        elif r.get("track_id") is not None:
            resolved_track_id = r.get("track_id")
