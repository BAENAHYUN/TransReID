...
def _load_checkpoint(weight_path: str):
    """
    체크포인트 파일 로드 — 두 가지 형식 지원:
      1) 일반 PyTorch .pth  (torch.save로 저장)
      2) zip 아카이브 안에 *.pth 가 들어있는 파일
         (Google Drive 등에서 내려받으면 이 형식으로 오는 경우가 있음)
    """
    import zipfile, io
    ...
    if zipfile.is_zipfile(weight_path):
        ...
        with zipfile.ZipFile(weight_path, "r") as z:
            ...
            if not pth_names:
                raise RuntimeError(f"zip 안에 .pth 파일이 없습니다: {weight_path}")
            target = sorted(pth_names)[0]   # best.pth 등
            print(f"    추출: {target}")
